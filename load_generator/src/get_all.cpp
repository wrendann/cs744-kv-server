...
void thread_get_all(std::atomic<bool> &stop_flag, LoadTestResult* result,
...
{
    std::cout << "Starting GET ALL Load Test with " << thread_num << " threads for " << duration_seconds << " seconds.\n";
    ...
    for (int i = 0; i < thread_num; ++i) {
        ...
        threads.emplace_back(thread_get_all, std::ref(stop_flag), results[i], std::cref(key_value_store));
    }
    ...
    std::cout << "GET ALL Load Test Completed: \n";
